...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchRepositories = async () => {
      try {
        const response = await fetch(`https://codevault-backend-d6bb.onrender.com/repo/user/${userId}`);
    ...
    const fetchSuggestedRepositories = async () => {
      try {
        const response = await fetch(`https://codevault-backend-d6bb.onrender.com/repo/all`);
        const data = await response.json();
