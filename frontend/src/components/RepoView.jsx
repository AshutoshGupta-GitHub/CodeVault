...
const RepoView = () => {
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      try {
        ...
        const response = await fetch(`https://codevault-backend-d6bb.onrender.com/repo/files/${decodedName}`);
  ...
  const handleFileClick = async (fileName) => {
      ...
      try {
        const response = await fetch(`https://codevault-backend-d6bb.onrender.com/repo/file-content/${repoName}/${fileName}`);
        const data = await response.json();
  ...
  const handleBrowserUpload = async (fileList) => {
      ...
      try {
        ...
        const response = await fetch(`https://codevault-backend-d6bb.onrender.com/upload-files/${repoName}`, {
          method: "POST",
          body: formData,
        });
